...
#define Base2_pin 5 //hakan (idk if this will be an available pin)
...
Base2.attach(Base2_pin); //hakan

}

int Arm(float x, float y, float z, int g, float wa, int wr, float b) //Here's all the Inverse Kinematics to control the arm (hakan, added float b for base2)
{
  ...
  Base.write(z); //here it is aiohewjsfdnc
...
void loop()
{
  ...
  if(LSX > Deadzone || LSX < -Deadzone)
  {
    ...
    tmpb = constrain(B + LSX/100.0*Speed, 0, 180); //hakan together i'm trying to have the left stick rotate both at the same time
  }
 ...
 if(Arm(tmpx, tmpy, tmpz, tmpg, tmpwa, tmpwr, tmpb)) //hakan
   {
     #ifdef DEBUG
 ...
 if(tmpx < 2 && tmpy < 2 && RSX < 0)
   {
     tmpy = tmpy + 0.05;
     Arm(tmpx, tmpy, tmpz, tmpg, tmpwa, tmpwr, tmpb); //hakan
   }
